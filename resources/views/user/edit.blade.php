...
<div class="flex flex-col gap-3 mx-8 md:w-1/3 md:mx-auto">
    ...
    <form action="{{ route('user.update') }}" method="POST" enctype="multipart/form-data" class="flex flex-col">
        ...
        <div class="flex flex-col items-center gap-4 mt-8">
            <div class="relative">
                <img id="imageProfil" src="{{ Auth::user()->photo_de_profil ? asset('storage/' . Auth::user()->photo_de_profil) : asset('assets/default_pfp.png') }}" alt="Avatar" class="rounded-full size-20">
        ...
        <div id="tab1" class="hidden mt-4 tab-content">
            <div class="flex flex-col gap-4">
                <div class="flex flex-col">
                    <label for="username" class="mb-1 text-gray-500">Nom d'utilisateur</label>
                    <input type="text" name="username" id="username" class="w-full px-3 py-2 text-sm border border-gray-200 rounded-md focus:outline-none" placeholder="Entrez un nom d'utilisateur">
                    @error('username')
                ...
                <div class="flex flex-col">
                    ...
                    <textarea name="description" id="description" rows="5" class="w-full px-3 py-2 text-sm border border-gray-200 rounded-md focus:outline-none" placeholder="Entrez une description"></textarea>
                    @error('description')
                        <div class="text-sm text-error-500">{{ $message }}</div>
                    @enderror
                </div>
        ...
        <div id="tab2" class="hidden mt-4 mb-24 tab-content">
            <div class="flex flex-col gap-4">
                <div class="flex flex-col">
                    ...
                    <input type="email" name="email" id="email" class="w-full px-3 py-2 text-sm border border-gray-200 rounded-md focus:outline-none" placeholder="Entrez votre adresse e-mail">
                    @error('email')
                        <div class="text-sm text-error-500">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <form action="{{ route('user.updatePassword') }}" method="POST">
                        @csrf
                        <button type="submit" class="w-full px-3 py-2 text-sm text-white rounded-md bg-primary-500 focus:outline-none">Modifier le mot de passe</button>
                    </form>
                </div>
                ...
                <div class="flex flex-col">
                    ...
                    <div class="flex gap-4">
                        <div class="flex flex-col w-1/2">
                            ...
                            <input type="date" name="date_naissance" id="date_naissance" class="w-full px-3 py-2 text-sm border border-gray-200 rounded-md focus:outline-none">
                            @error('date_naissance')
                                <div class="text-sm text-error-500">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="flex flex-col w-1/2">
                            <label for="carte_identite" class="mb-1 text-sm text-gray-500">Carte d'identité</label>
                    ...
                    <div class="flex flex-col mt-2">
                        ...
                        <input type="text" name="localisation" id="localisation" class="w-full px-3 py-2 text-sm border border-gray-200 rounded-md focus:outline-none" placeholder="Entrez votre localisation">
                        @error('localisation')
                            <div class="text-sm text-error-500">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="flex flex-col mt-2">
                        ...
                        <input type="text" name="telephone" id="telephone" class="w-full px-3 py-2 text-sm border border-gray-200 rounded-md focus:outline-none" placeholder="Entrez votre numéro de téléphone">
                        @error('telephone')
                            <div class="text-sm text-error-500">{{ $message }}</div>
                        @enderror
                    </div>
