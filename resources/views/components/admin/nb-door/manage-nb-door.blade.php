<div x-data="{ open: false, nb_door_id: @entangle('nb_door_id') }" @open-manage-nb-door-modal.window="open = true; nb_door_id = $event.detail.nb_door_id; $wire.openModal(nb_door_id)" @close-manage-nb-door-modal.window="open = false">
    <button @click="open = true; $wire.openModal()" class="px-4 py-2 text-white rounded-md bg-primary-500 hover:bg-primary-600"><i class="fa-solid fa-plus"></i> Ajouter un nombre de portières</button>
        ...
        <div class="w-full max-w-md p-6 bg-white rounded-lg shadow-lg" @click.stop>
            ...
            <form wire:submit.prevent="saveNbDoor">
                <div class="mb-4">
                    ...
                    <input type="text" id="nb_doors" wire:model="nb_doors" class="w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-primary-500 focus:ring-primary-500">
                ...
                <div class="flex justify-end gap-4">
                    ...
                    <button type="submit" class="px-4 py-2 text-sm text-white transition-all duration-300 rounded-lg bg-primary-500 hover:bg-primary-600" x-text="nb_door_id ? 'Modifier' : 'Ajouter'"></button>
                </div>
            </form>
        </div>
    </div>
