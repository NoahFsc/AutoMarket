<div x-data="{ open: false, gearbox_id: @entangle('gearbox_id') }" @open-manage-gearbox-modal.window="open = true; gearbox_id = $event.detail.gearbox_id; $wire.openModal(gearbox_id)" @close-manage-gearbox-modal.window="open = false">
    <button @click="open = true; $wire.openModal()" class="px-4 py-2 text-white rounded-md bg-primary-500 hover:bg-primary-600"><i class="fa-solid fa-plus"></i> Ajouter une boîte de vitesse</button>
        ...
        <div class="w-full max-w-md p-6 bg-white rounded-lg shadow-lg" @click.stop>
            ...
            <form wire:submit.prevent="saveGearbox">
                ...
                <div class="flex justify-end gap-4">
                    ...
                    <button type="submit" class="px-4 py-2 text-sm text-white transition-all duration-300 rounded-lg bg-primary-500 hover:bg-primary-600" x-text="gearbox_id ? 'Modifier' : 'Ajouter'"></button>
                </div>
            </form>
        </div>
    </div>
