        ...
        <table class="min-w-full bg-gray-300 rounded-lg">
            ...
            <tbody class="bg-white">
                ...
                <tr class="border-b border-gray-300">
                    <td class="px-6 py-4">
                        <div class="text-sm font-medium text-gray-900">{{ $vehicleType->nom }}</div>
                    </td>
                    <td class="px-6 py-4">
                        <div class="text-sm font-medium text-gray-900">{{ $vehicleType->segment }}</div>
                    </td>
                        ...
                        <div class="relative inline-block text-left" x-data="{ open: false }">
                            ...
                            <div x-cloak x-show="open" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="opacity-0 transform scale-95" x-transition:enter-end="opacity-100 transform scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="opacity-100 transform scale-100" x-transition:leave-end="opacity-0 transform scale-95" @click.away="open = false" class="absolute right-0 z-50 w-56 mt-2 bg-white rounded-md shadow-lg ring-1 ring-black ring-opacity-5">
                                <div class="py-1" role="menu">
                                    <button @click="$dispatch('open-manage-type-modal', { vehicle_type_id: {{ $vehicleType->id }} })" class="block w-full px-4 py-2 text-sm text-left text-gray-700 hover:bg-gray-100" role="menuitem">Modifier le type de véhicule</button>
                                    <button wire:click="deleteType({{ $vehicleType->id }})" class="block w-full px-4 py-2 text-sm text-left text-red-500 hover:bg-gray-100" role="menuitem">Supprimer le type de véhicule</button>
                                </div>
                            </div>
                        </div>
                    </td>
