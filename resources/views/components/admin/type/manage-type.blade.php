<div x-data="{ open: false, vehicle_type_id: @entangle('type_of_car_id') }" @open-manage-type-modal.window="open = true; vehicle_type_id = $event.detail.vehicle_type_id; $wire.openModal(vehicle_type_id)" @close-manage-type-modal.window="open = false">
    <button @click="open = true; $wire.openModal()" class="px-4 py-2 text-white rounded-md bg-primary-500 hover:bg-primary-600"><i class="fa-solid fa-plus"></i> Ajouter un type de véhicule</button>
        ...
        <div class="w-full max-w-md p-6 bg-white rounded-lg shadow-lg" @click.stop>
            ...
            <form wire:submit.prevent="saveType">
                <div class="mb-4">
                    ...
                    <input type="text" id="segment" wire:model="segment" class="w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-primary-500 focus:ring-primary-500">
                ...
                <div class="flex justify-end gap-4">
                    ...
                    <button type="submit" class="px-4 py-2 text-sm text-white transition-all duration-300 rounded-lg bg-primary-500 hover:bg-primary-600" x-text="vehicle_type_id ? 'Modifier' : 'Ajouter'"></button>
                </div>
            </form>
        </div>
    </div>
