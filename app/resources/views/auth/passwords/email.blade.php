...
<div class="flex flex-col mx-8 md:w-1/3 md:mx-auto">
    ...
    <div class="text-base text-center text-default/50">Pas de problème, nous vous enverrons les instructions par e-mail.</div>
    <form method="POST" action="{{ route('password.email') }}" class="flex flex-col w-full gap-4 mt-4 md:mt-6">
        ...
        <div class="flex flex-col">
            ...
            <input type="email" name="email" id="email" class="w-full px-3 py-2 text-sm border rounded-md border-input-border bg-input focus:outline-none" placeholder="Entrez votre email" required>
            @error('email')
