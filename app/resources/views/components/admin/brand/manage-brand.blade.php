<div x-data="{ open: false, brand_id: @entangle('brand_id') }" @open-manage-brand-modal.window="open = true; brand_id = $event.detail.brand_id; $wire.openModal(brand_id)" @close-manage-brand-modal.window="open = false">
    <button @click="open = true; $wire.openModal()" class="px-4 py-2 text-white rounded-md bg-primary hover:bg-opacity-80"><i class="fa-solid fa-plus"></i> Ajouter une marque</button>
        ...
        <div class="w-full max-w-md p-6 bg-white rounded-lg shadow-lg" @click.stop>
            ...
            <form wire:submit.prevent="saveBrand">
                <div class="mb-4">
                    ...
                    <input type="text" id="brand_name" wire:model="brand_name" class="w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-primary focus:ring-primary">
