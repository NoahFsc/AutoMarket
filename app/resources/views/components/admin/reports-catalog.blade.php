<div class="relative flex flex-col flex-grow gap-4 mt-16 mr-16">
    ...
    <div class="flex flex-col">
        <p class="text-2xl font-medium">{{ __('ReportManagement') }}</p>
        <p class="text-lg font-medium opacity-50">{{ __('ManageReports') }}</p>
    </div>

    {{-- Barre d'outils --}}
    <div class="flex items-end justify-between">
        <p class="font-medium">{{ __('AllReports') }} <span class="font-medium opacity-50">({{ $reports->total() }})</span></p>
        <div>
    ...
    <div class="overflow-x-auto">
        ...
        <caption id="reportsTableDescription" class="sr-only">{{ __('Tableau listant tous les signalements des utilisateurs avec des options pour marquer comme traité, en traitement ou supprimer chaque signalement.') }}</caption>
            <thead>
                <tr>
                    <th class="px-6 py-3 font-medium text-left">{{ __('ReportedUser') }}</th>
                    <th class="px-6 py-3 font-medium text-left">{{ __('ReportedBy') }}</th>
                    <th class="px-6 py-3 font-medium text-left">{{ __('Message') }}</th>
                    <th class="px-6 py-3 font-medium text-left">{{ __('Status') }}</th>
                ...
                <tr class="border-b border-input-border bg-input/80">
                    ...
                    <td class="px-6 py-4">
                        <span class="inline-flex px-2 text-xs font-semibold {{ $report->status ? 'text-green-800 bg-green-100' : 'text-yellow-800 bg-yellow-100' }} rounded-full">
                            {{ $report->status ? __('Resolved') : __('InProgress') }}
                        </span>
                    </td>
                        ...
                        <div class="relative inline-block text-left" x-data="{ open: false }">
                            ...
                            <div x-cloak x-show="open" @click.away="open = false" class="absolute right-0 z-50 w-56 mt-2 rounded-md shadow-lg bg-input ring-1 ring-black ring-opacity-5" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="opacity-0 transform scale-95" x-transition:enter-end="opacity-100 transform scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="opacity-100 transform scale-100" x-transition:leave-end="opacity-0 transform scale-95">
                                <div class="py-1" role="menu">
                                    <button wire:click="markAsResolved({{ $report->id }})" class="block w-full px-4 py-2 text-sm text-left text-green-500 hover:bg-default/5" role="menuitem">{{ __('MarkAsResolved') }}</button>
                                    <button wire:click="markAsUnresolved({{ $report->id }})" class="block w-full px-4 py-2 text-sm text-left text-yellow-500 hover:bg-default/5" role="menuitem">{{ __('MarkAsInProgress') }}</button>
                                    <button wire:click="deleteReport({{ $report->id }})" class="block w-full px-4 py-2 text-sm text-left text-red-500 hover:bg-default/5" role="menuitem">{{ __('DeleteReport') }}</button>
                                </div>
                            </div>
                        </div>
                    </td>
