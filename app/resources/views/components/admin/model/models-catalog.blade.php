<div class="relative flex flex-col flex-grow gap-4 mt-16 mr-16">
    ...
    <div class="flex flex-col">
        <p class="text-2xl font-medium">Gestion des modèles</p>
        <p class="text-lg font-medium opacity-50">Gérez les modèles disponibles sur AutoMarket</p>
    </div>

    {{-- Barre d'outils --}}
    <div class="flex items-end justify-between">
        <p class="font-medium">Tous les modèles <span class="font-medium opacity-50">({{ $models->total() }})</span></p>
    ...
    <div class="overflow-x-auto">
        ...
        <caption id="modelsTableDescription" class="sr-only">Tableau listant tous les modèles disponibles sur AutoMarket avec des options pour modifier ou supprimer chaque modèle.</caption>
            <thead>
                <tr>
                    <th class="px-6 py-3 font-medium text-left">Nom du modèle</th>
                        ...
                        <div class="relative inline-block text-left" x-data="{ open: false }">
                            ...
                            <div x-cloak x-show="open" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="opacity-0 transform scale-95" x-transition:enter-end="opacity-100 transform scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="opacity-100 transform scale-100" x-transition:leave-end="opacity-0 transform scale-95" @click.away="open = false" class="absolute right-0 z-50 w-56 mt-2 rounded-md shadow-lg bg-input ring-1 ring-black ring-opacity-5">
                                <div class="py-1" role="menu">
                                    <button @click="$dispatch('open-manage-model-modal', { model_id: {{ $model->id }} })" class="block w-full px-4 py-2 text-sm text-left text-default/80 hover:bg-default/5" role="menuitem">Modifier le modèle</button>
                                    <button wire:click="deleteModel({{ $model->id }})" class="block w-full px-4 py-2 text-sm text-left text-red-500 hover:bg-default/5" role="menuitem">Supprimer le modèle</button>
                                </div>
                            </div>
                        </div>
                    </td>
